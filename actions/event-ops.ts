 ...
 *
 * Render + send live in sendRunSheetCore (lib/ops/event-ops.ts, inc-3 B5) so
 * the evening-before cron sends the identical email; this action contributes
 * the gate and the recipient identity (always the caller's own member email).
 */
export async function sendRunSheet(orgId: string, eventId: string): Promise<{ to: string }> {
  ...
  return sendRunSheetCore(orgId, eventId, { email: member.email })
}
