...
// The original full-seed body of scripts/seed-demo.ts, moved verbatim behind a
// dynamic import so the entry point can parse args (and print --dry-run plans)
// without initializing firebase-admin. Run via `npm run seed:demo` — it sets
// --conditions=react-server so 'server-only' (imported transitively via
// lib/firebase-admin) resolves to its no-throw module under tsx.
...
export async function resolveDemoUser(args: { email: string; password: string }): Promise<UserRecord> {
  try {
 ...
 */
export function assertClaimsAreDemoOwned(user: UserRecord, email: string): void {
 ...
 * data in the target project. `runFullSeed()` must not reset, write, or set claims until
 * this returns.
 *
...
export async function runFullSeed(args: SeedArgs): Promise<void> {
