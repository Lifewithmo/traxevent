...
const resolveCtx = cache((orgSlug: string, eventSlug: string) =>
  requireEventPage(orgSlug, eventSlug, 'assignments')
)
...
export async function generateMetadata({
  ...
  const { event } = await resolveCtx(orgSlug, eventSlug)
...
export default async function AssignmentsPrintPage({
  ...
  const { orgSlug, eventSlug } = await params
  const { orgId, eventId, event } = await resolveCtx(orgSlug, eventSlug)
