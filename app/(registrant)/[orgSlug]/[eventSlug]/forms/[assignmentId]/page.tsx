...
export default function FormFillPage() {
  const { orgSlug, eventSlug, assignmentId } = useParams<{
    orgSlug: string
    eventSlug: string
    assignmentId: string
  }>()
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!o) return
      const c = await getEventBySlug(o.id, eventSlug)
      if (!c) return
      setOrg(o)
      setEvent(c)
    ...
    load()
  }, [orgSlug, eventSlug, assignmentId, token])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(
        `/${orgSlug}/${eventSlug}/my-registration${token ? `?token=${token}` : ''}`
      )
    } catch (err: unknown) {
  ...
  if (alreadySigned) {
    return (
      <div className="space-y-4">
        ...
        <Link href={`/${orgSlug}/${eventSlug}/my-registration${token ? `?token=${token}` : ''}`}>
