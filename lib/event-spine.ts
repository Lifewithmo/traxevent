...
import { parseDay } from '@/lib/event-ui'
import {
...
export interface EventSpineKpis {
  ...
  /** Closeout-doc presence for the wrapped-state NBA (B9). null = ops gated off or read failed. */
...
export interface GetEventSpineKpisInput {
  orgId: string
  eventId: string
  event: Pick<Event, 'event_start' | 'hours' | 'lead_id'>
 ...
 */
export async function getEventSpineKpis({ orgId, eventId, event, allowedPages, includeMoney, today }: GetEventSpineKpisInput): Promise<EventSpineKpis> {
  const todayDay = today ?? new Date().toISOString().slice(0, 10)
  const kpis: EventSpineKpis = {
    registrations: null, financial: null, readiness: null,
    ops: null, ar: null, closeout: null, firstItineraryTime: null, blockers: [],
  }
  let plan: OpsPlan | null = null
  ...
  await Promise.all([
    (async () => {
      ...
      try {
        ...
        if (includeMoney) kpis.financial = buildFinancialReport(families)
      } catch {
        // section stays null
      }
    })(),
    (async () => {
      if (allowedPages.includes('ops')) {
        try {
          plan = await getOpsPlanCore(orgId, eventId)
          kpis.ops = planFacts(plan)
          if (plan) kpis.readiness = computeReadiness(plan, event.event_start)
        } catch {
          // section stays null
        }
      }
      // Itinerary time fallback (B7 precedence): only read when neither the
      // plan's service_start nor the event's hours can answer, and the member
      // may see the itinerary. Mirrors actions/itinerary.ts listItinerary
      // (bare .get(); ordering happens in memory).
      if (allowedPages.includes('itinerary') && !event.hours?.start && !plan?.requirements?.service_start) {
        try {
          const snap = await eventRef.collection('itinerary').get()
          ...
          kpis.firstItineraryTime = first?.start_time ?? null
        } catch {
          // stays null → job strip says 'Time TBD'
        }
      }
    })(),
    (async () => {
      ...
      try {
        ...
        kpis.closeout = { exists: closeout != null, completed: closeout?.completed ?? false }
      } catch {
        // section stays null
      }
    })(),
    (async () => {
      ...
      if (!includeMoney || !event.lead_id) return
      try {
        const invoices = await listInvoicesCore(orgId, event.lead_id)
        kpis.ar = buildArSummary(invoices, new Date(`${todayDay}T12:00:00Z`))
      } catch {
        // section stays null
      }
    })(),
  ...
  kpis.blockers = computeEventBlockers({ plan, registrations: kpis.registrations, ar: kpis.ar, today: todayDay })
  return kpis
}
...
export function computeEventBlockers(input: {
  ...
  const pending = registrations?.byStatus['pending'] ?? 0
  if (pending > 0) {
...
export type EventNbaTarget = 'ops' | 'ops/loadout' | 'ops/runsheet' | 'ops/closeout' | 'families' | 'lead-invoices'
...
export interface EventNba {
  ...
  /** true = blockers[0] was promoted into this button (B9) — render the remaining blockers only. */
  fromTopBlocker: boolean
}

const BLOCKER_TARGET: Record<EventBlocker['kind'], EventNbaTarget> = {
  deadline: 'ops',
  money: 'lead-invoices',
  loadlist: 'ops/loadout',
  registrations: 'families',
}

/** The blocker's deep-link target, shared by blocker rows and the promoted NBA. */
export function blockerTarget(kind: EventBlocker['kind']): EventNbaTarget {
  return BLOCKER_TARGET[kind]
}

/**
 * THE single next-best-action (S1.4, B9): wrapped → close out; day-of → run
 * sheet; no plan → instantiate; otherwise the top blocker promoted to the
 * primary button; fully ready → run sheet. Never two competing next-things.
 */
export function computeEventNba(input: {
  ...
  const { phase, ops, closeout, blockers } = input
  if (!ops) {
    ...
    const b = blockers[0]
    return b ? { label: b.actionLabel, target: BLOCKER_TARGET[b.kind], fromTopBlocker: true } : null
  }
  if (phase === 'wrapped') {
  ...
  const b = blockers[0]
  if (b) return { label: b.actionLabel, target: BLOCKER_TARGET[b.kind], fromTopBlocker: true }
  return { label: 'Open run sheet', target: 'ops/runsheet', fromTopBlocker: false }
}
