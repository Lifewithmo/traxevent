...
describe('canAccessEventPage', () => {
  ...
  it('staff can access only granted pages for the specific camp', () => {
    ...
    expect(canAccessEventPage(m, 'camp-1', 'families')).toBe(true)
    expect(canAccessEventPage(m, 'camp-1', 'budget')).toBe(false)
  })

  it('staff with no entry for the camp is denied', () => {
    const m = member({ role: 'staff', event_access: { 'camp-2': { pages: ['families'] } } })
    expect(canAccessEventPage(m, 'camp-1', 'families')).toBe(false)
  })
})
...
describe('canAccessEventPage — department inheritance', () => {
  it('grants access when the member has a department grant for the camp\'s department', () => {
    ...
    expect(canAccessEventPage(m, 'camp-1', 'budget', 'dept-1')).toBe(false)
  })
  ...
  it('does not grant a department page when the camp is in a different/absent department', () => {
    ...
    expect(canAccessEventPage(m, 'camp-1', 'forms', 'dept-2')).toBe(false)
    expect(canAccessEventPage(m, 'camp-1', 'forms', null)).toBe(false)
    expect(canAccessEventPage(m, 'camp-1', 'forms')).toBe(false)
  })

  it('explicit camp grant still works regardless of department', () => {
    const m = member({ role: 'staff', event_access: { 'camp-1': { pages: ['families'] } } })
    expect(canAccessEventPage(m, 'camp-1', 'families', 'dept-1')).toBe(true)
  })
