...
vi.mock('@/lib/firebase-admin', () => {
  ...
  const eventsCol = { orderBy: vi.fn().mockReturnValue({ get: listEventsGetSpy }), doc: vi.fn().mockReturnValue(eventDoc) }
...
describe('getOrgHouseholds', () => {
  ...
  it('aggregates families across camps into deduped households', async () => {
    listEventsGetSpy.mockResolvedValue({ docs: [event('c1', 2025), event('c2', 2026)] })
    familiesGetSpy
