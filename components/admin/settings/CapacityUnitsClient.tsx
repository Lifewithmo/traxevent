...
interface CapacityUnitsClientProps {
  orgId: string
  /** For the pointer row to the Event Types settings page. */
  orgSlug: string
...
export function CapacityUnitsClient({
  ...
  return (
    <div className="max-w-2xl space-y-6 p-6">
      ...
      {/* Event types graduated to their own settings page (event types inc 1,
          spec §5a) — one source of truth for the list every picker reads. */}
      <p className="rounded-lg bg-muted/50 px-3 py-2 text-sm text-muted-foreground">
        Event types moved — manage them in{' '}
        <Link
